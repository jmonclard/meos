...
wstring getMeosDate() {
  wstring date(L"$Date: 2024-10-15 23:45:00 +0200 $");
...
wstring getMeosFullVersion() {
  ...
  if (getBuildType().empty())
    swprintf_s(bf, L"Version (JM) X#%s.%d (%s), %s", maj.c_str(), getMeosBuild(), bits, getMeosDate().c_str());
  else
    swprintf_s(bf, L"Version (JM) X#%s.%d (%s), %s, %s", maj.c_str(), getMeosBuild(), bits, getBuildType().c_str(), getMeosDate().c_str());
  return bf;
}
